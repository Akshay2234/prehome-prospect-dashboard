...
const Login = () => {
  ...
  const handleLogin = async () => {
    try {
      const response = await axios.post("http://35.154.52.56:5000/api/auth/login", { email, password });
  ...
  const handleGoogleLoginSuccess = async (credentialResponse) => {
    try {
      ...
      const response = await axios.post("http://35.154.52.56:5000/api/auth/google-login", { email: decoded.email });
      localStorage.setItem("token", response.data.token);
      localStorage.setItem("user_id", response.data.userId);
  ...
  const handleFacebookResponse = async ({ provider, data }) => {
    try {
      const fbResponse = await axios.post("http://35.154.52.56:5000/api/auth/facebook-login", {
        userID: data.id
      });
  ...
  const handleForgotSubmit = async () => {
    try {
      const res = await axios.post("http://35.154.52.56:5000/api/auth/forgot-password", { email: forgotEmail });
  ...
  const handleVerifyOtp = async () => {
    try {
      const res = await axios.post("http://35.154.52.56:5000/api/auth/verify-reset-otp", { email: forgotEmail, otp });
  ...
  const handleResetPassword = async () => {
    try {
      const res = await axios.post("http://35.154.52.56:5000/api/auth/reset-password", { email: forgotEmail, password: newPassword });
